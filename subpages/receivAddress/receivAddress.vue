<template>
	<view>
		...
		<view>
			<view class="view_shadow list" v-for="(item, index) in addList" @click="selectAdress(index)">
				<view class="left">
...
<script>
	...
	export default {
		...
		methods: {
			selectAdress(index) { // 选中地址 传该地址的参数， 然后销毁该页面
				uni.redirectTo({
					url: "/subpages_shop/orderPay/orderPay/?addrId=" + this.addList[index].addrId
				})
			},
			toEdit() {
				uni.navigateTo({
