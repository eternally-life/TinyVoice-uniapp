	...
	<view class="wrap">
		...
		<view class="banner">
			<u-swiper
				...
				indicatorActiveColor="rgba(45,193,207,0.7)"
				indicator
				indicatorMode="dot"
				height="150"
				previousMargin="30"
		...
		<view class="centerInfo" v-if="navs.length > 0">
			<view
				class="myItem"
				@click="navItemClick(item.jumpUrl, item.appId, item.jumpType)"
				v-for="(item, index) in navsWeb"
				:key="index"
			>
				<view class="myIcon">
					<view class="fix" v-if="item.iconType == 1" :class="item.icon"></view>
					<view class="fix" v-if="item.iconType == 2"><image :src="item.image" mode="widthFix" /></view>
				</view>
				<text>{{ item.name }}</text>
			</view>
		</view>
...
import { systemSyscarouselPage_Get, toolsList_Get } from '@/api/SYSTEM/轮播图.js';
export default {
	mixins: [tabbar_hid],
	data() {
		return {
		...
		getBanner() {
			systemSyscarouselPage_Get({
				...
				if (res.data.code == 200 && res.data.data.records.length > 0) {
					let temp = [].concat(res.data.data.records);
